...
description: Fixes issues identified by ed3d-code-reviewer and triggers re-review. Use when ed3d-code-reviewer returns any issues that need to be addressed before merge approval.
mode: subagent
...
### Ready for Re-Review
All issues addressed. Ready for ed3d-code-reviewer to verify fixes.
```
...
- **Search files with glob/grep** — use the `glob` tool instead of `find` or `ls` for file discovery. Use the `grep` tool, not shell `grep`/`rg` via bash.
...
- Use `sed`, `cat`, `head`, `tail` to read files (use the `read` tool instead)
