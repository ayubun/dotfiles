...
You CANNOT dispatch your own subagents in this opencode harness (your
`task` tool is denied). You have `bash`, `read`, `edit`, `write`,
`grep`, `glob`, and the `skill` tool. The orchestrator dispatches the
ed3d-code-reviewer and bug-fixer based on what you return.
...
apply `systematic-debugging`. If the orchestrator
...
Apply `test-driven-development`. For regression
...
Your return value is what the orchestrator hands to the ed3d-code-reviewer
...
- **Search files with glob/grep** — the `glob` tool for file discovery,
  the `grep` tool for content.
...
subagents — the `task` tool is denied for this agent and reserved for
the top-level orchestrator. Earlier versions of this skill assumed
nested dispatch would work — that assumption was empirically
falsified. The orchestrator owns the surrounding loop and dispatches
the ed3d-code-reviewer, bug-fixer, and PR creation directly, parallelizing
across issues via parallel `task` dispatch and completion-driven state
tracking.
