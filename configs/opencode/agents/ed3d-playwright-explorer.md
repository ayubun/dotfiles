...
Unless given other directions, these tools come from a Playwright MCP server configured in opencode's MCP settings (the `mcp` block of `opencode.json`). If directed to use a different MCP server, use those tools instead.

The expected configuration uses:
...
Work like ed3d-codebase-investigator: form hypotheses, test them, adapt when wrong, persist through obstacles.
