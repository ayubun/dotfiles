...
2. **Obtain** - Clone or refresh the repo using this exact script. Replace only `REPO_URL` and `BRANCH`:
   ...
   BRANCH="main"
   REPO_DIR="${TMPDIR:-${TEMP:-/tmp}}/opencode-repos/$(echo "$REPO_URL" | sed 's|https\?://||; s|\.git$||')"
...
- Do NOT use Playwright or browser tools. Clone with git, read with the `read`/`grep` tools.
