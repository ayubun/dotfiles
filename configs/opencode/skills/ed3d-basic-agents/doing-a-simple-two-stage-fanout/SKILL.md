...
Divide a corpus across Worker subagents, review with Critic subagents, synthesize with a Summarizer. Every stage writes to files; every subagent gets its own todo item.
...
If the user's intent is not already clear, ask two questions using the `question` tool:
...
For more precise estimates, run the [compute_layout.py](./compute_layout.py) script shipped with this skill (its absolute path is shown in the file list when this skill loads):
...
## Step 3: Write the Orchestration Plan and Get Approval
...
Present the plan to the user and ask for explicit approval (use the `question` tool). Do not proceed until the user approves the plan.
...
Before launching any subagents, write the full todo list upfront using the `todowrite` tool:

- One item per Worker (`W01`, `W02`, ...)
- One item per Critic (`C01`, `C02`, ...)
- One item for the Summarizer

List the items in execution order: all Workers, then all Critics, then the Summarizer. Todo items carry no dependency links, so the ordering plus these rules encode the dependency graph: a Critic may start only after every Worker whose segments it reviews has completed, and the Summarizer may start only after all Critics have completed.

This lays out the full pipeline before any work starts.

## Step 5: Launch Workers

Mark the Worker todo items as `in_progress`, then launch all Workers in parallel (one `task` tool call per Worker, all in the same message so they run in parallel; react to completion notifications — do not poll or sleep).

### Worker Dispatch Template

Dispatch each Worker like this:

```
task:
  subagent_type: ed3d-sonnet-general-purpose
  description: {WORKER_NAME} analyzes segments {FIRST_SEG}-{LAST_SEG}
  prompt: |
...
Mark completed Workers' todo items as `completed`. If any Worker failed or did not produce output, see **Failure Recovery** below.

## Step 6: Launch Critics

Mark the Critic todo items as `in_progress`, then launch all Critics in parallel (one `task` tool call per Critic, all in the same message).
...
task:
  subagent_type: ed3d-sonnet-general-purpose
  description: {CRITIC_NAME} reviews segments {SEG_RANGE}
  prompt: |
...
Verify output files and mark their todo items `completed`. Handle failures per **Failure Recovery**.

## Step 7: Launch Summarizer

Mark the Summarizer todo item as `in_progress`. Launch a single Summarizer subagent:

```
task:
  subagent_type: ed3d-sonnet-general-purpose
  description: Summarizer synthesizes the final report
  prompt: |
...
4. Add new todo items for the split agents, slotted into the original's position in the pipeline order (the same predecessors and successors apply).
5. Mark the original todo item as `completed` (it was replaced, not failed).
...
| Default agent type | ed3d-sonnet-general-purpose | ed3d-sonnet-general-purpose | ed3d-sonnet-general-purpose |
...
Track every item below with the `todowrite` tool. Mark each `in_progress` before starting it, `completed` after finishing. Do not skip items or batch them.

- [ ] Ask user for analysis goal (or confirm from context)
- [ ] Ask user for effort level
- [ ] Estimate corpus size (use `wc -c` via the `bash` tool for character counts)
- [ ] Compute layout (run compute_layout.py or use `python3 -c "..."` via the `bash` tool — never mental math)
...
- [ ] Write orchestration plan with diagram, assignment tables, and file layout
- [ ] Get user approval of the plan (use the `question` tool)
- [ ] Write the full todo list (one item per Worker, Critic, Summarizer) in execution order
